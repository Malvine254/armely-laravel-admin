        ...
        <div ref="bulkActionsBarRef" class="px-4 sm:px-5 py-3 border-b flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3" style="border-color: #d9e6f7; background: #f9fcff;">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <button
              ...
              :disabled="selectedQuoteIds.length !== 1 || !hasRevisableSelectedQuote || !!processingQuoteId"
            ...
            >
              {{ selectedQuoteIds.length > 1 ? 'Select 1 Quote' : (!hasRevisableSelectedQuote ? 'Not Revisable' : 'Revise Selected') }}
            </button>
            <button
...
export default {
  ...
  setup() {
    ...
    const hasRevisableSelectedQuote = computed(() => {
      return selectedQuotes.value.length === 1 && canReviseQuote(selectedQuotes.value[0])
    })
    ...
    const reviseSelectedQuotes = async () => {
      ...
      if (!canReviseQuote(quote)) {
        const status = String(quote?.status || '').toLowerCase()
        const message = ['cancelled', 'rejected'].includes(status)
          ? `${formatStatus(status)} quotes cannot be revised`
          : 'This quote cannot be revised'
        toastStore.addToast(message, 'warning', 3000, { category: 'quotes' })
        return
      }
    ...
    const reviseQuote = async (quote) => {
      ...
      if (!canReviseQuote(quote)) {
        ...
        if (['cancelled', 'rejected'].includes(status)) {
          toastStore.addToast(`${formatStatus(status)} quotes cannot be revised. Create a new quote from products instead.`, 'warning', 3000, { category: 'quotes' })
        } else {
