...
class AuthController extends Controller
{
    ...
    private function buildProfilePictureUrl(?string $path): ?string
    {
        ...
        // Use the request's actual scheme+host first so localhost vs 127.0.0.1
        // does not create cross-origin image URLs that are blocked by CSP.
        if ($baseUrl === '') {
            try {
                if (request()) {
    ...
    public function forgotPassword(Request $request): JsonResponse
    {
        ...
        \Log::warning('Azure Graph password reset email was not sent; SMTP fallback is disabled for this flow.', [
            'to' => $user->email,
        ]);
    ...
    private function sendActivationEmail(User $user, string $activationUrl): void
    {
        ...
        \Log::warning('Azure Graph activation email was not sent; SMTP fallback is disabled for this flow.', [
            'to' => $user->email,
        ]);
    }
}
