...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .form-control { border: 1px solid #d9e2ef; border-radius: 9px !important; background: #f8fafc; color: var(--ink); font-size: 14px; min-height: 46px; padding: 11px 14px 11px 44px; }
        ...
        .btn-auth {
            ...
            font-size: 15px; font-weight: 800; box-shadow: 0 10px 22px rgba(47,85,151,.24);
        }
        .btn-auth:hover { color:#fff; transform: translateY(-1px); box-shadow: 0 14px 26px rgba(47,85,151,.3); }
        .hint { color: var(--muted); font-size: 13px; margin: 0; line-height: 1.55; }
        ...
        <section class="auth-card">
            ...
            <form method="POST" action="{{ route('admin.password.update') }}">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input type="password" class="form-control @error('password') is-invalid @enderror" id="password" name="password" placeholder="New password" required autofocus>
                        <span class="icon"><i class="fas fa-lock"></i></span>
                    </div>
                    @error('password')<small class="text-danger d-block mt-2">{{ $message }}</small>@enderror
                    <p class="hint mt-2"><i class="fas fa-info-circle me-1"></i>Use at least 8 characters.</p>
                </div>
                ...
                <button type="submit" class="btn btn-auth"><i class="fas fa-check me-2"></i>Reset Password</button>
            </form>
        </section>
    </main>
